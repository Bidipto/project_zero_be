...
class CRUDChat(CRUDBase[Chat, ChatCreate, ChatUpdate]):
    ...
    def get_user_chats(
        ...
        *, 
        ...
        return db.query(Chat).join(Chat.participants).filter(
            and_(User.id == user_id, Chat.is_active == True)
    ...
    def get_private_chat(
        ...
        *, 
        ...
        chats = db.query(Chat).join(Chat.participants).filter(
            and_(
    ...
    def add_participant(self, db: Session, *, chat_id: int, user_id: int) -> Optional[Chat]:
    ...
    def remove_participant(self, db: Session, *, chat_id: int, user_id: int) -> Optional[Chat]:
    ...
    def get_chat_with_messages(
        ...
        *, 
        ...
        if chat:
            ...
            ).order_by(desc(Message.timestamp)).limit(message_limit).all()
